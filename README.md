...
By default, `--page-header markdown` renders the page title and linked description in the Markdown body. Documentation themes that provide their own page header can use `--page-header frontmatter` instead. That mode adds `crefdocs: true` and a linked `descriptionMarkdown` field to the frontmatter while retaining the plain `description` used by navigation and SEO metadata.
